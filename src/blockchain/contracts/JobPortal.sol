...
contract JobPortal{
    ...
    event NewProject(uint projectId, address projectManager);
    event NewTask(uint projectId, uint taskId, uint stakedAmount);
    event NewProposal(uint projectId, uint taskId, address freelancer, uint bidAmount);
    ...
    function getTaskData(uint projectId, uint taskId) public view returns (
        ...
        return (task.taskId, task.stakedAmount, task.proposalCount, task.selectedWorker, task.completed, task.reviewed);
    }


    function createTask(uint projectId, uint stakedAmount) public {
        ...
        emit NewTask(projectId, projects[projectId].taskCount-1, stakedAmount);
    }

    function sendProposal(uint projectId, uint taskId, uint bidAmount, string memory motivation) public {
